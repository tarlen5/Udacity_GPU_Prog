    ...
    )
{
  ...
  if ( image_index_2d.x < nx && image_index_2d.y < ny )
  {
    ...
    for ( int y = -template_half_height; y <= template_half_height; y++ )
    {
      ...
      {
        int2 image_offset_index_2d = make_int2( image_index_2d.x + x,
                                                image_index_2d.y + y );
    ...
    for ( int y = -template_half_height; y <= template_half_height; y++ )
    {
      for ( int x = -template_half_width; x <= template_half_width; x++ )
      {
        ...
        int2 template_index_2d = make_int2( x + template_half_width, y +
...
void preProcess(unsigned int **inputVals,
  ...
  unsigned int g_sum = thrust::reduce(d_green_template.begin(),
                                      d_green_template.end(), 0);
  ...
  thrust::pair<floatIt, floatIt> minmax =
    thrust::minmax_element(d_combined_response.begin(), d_combined_response.end());
