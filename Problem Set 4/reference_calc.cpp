...
void simple_print_to_file(unsigned int* inputVals,
                          ...
                          const size_t numElems)
{
  ...
  FILE* f = fopen("ref_inVals.out","w");
  for(unsigned ii=0; ii < numElems; ii++) {
    fprintf(f,"%u \n",inputVals[ii]);
  }
  fclose(f);

  f = fopen("ref_inPos.out","w");
  for(unsigned ii=0; ii < numElems; ii++) {
    fprintf(f,"%u \n",inputPos[ii]);
  }
  fclose(f);

  f = fopen("ref_outVals.out","w");
  for(unsigned ii=0; ii < numElems; ii++) {
    fprintf(f,"%u \n",outputVals[ii]);
  }
  fclose(f);

  f = fopen("ref_outPos.out","w");
  for(unsigned ii=0; ii < numElems; ii++) {
    fprintf(f,"%u \n",outputPos[ii]);
  }
  fclose(f);
                           ...
                           const size_t numElems)
{
  ...
  //simple_print_to_file(inputVals, inputPos, outputVals, outputPos, numElems);
  ...
  for (unsigned int i = 0; i < 8 * sizeof(unsigned int); i += numBits) {
    ...
    //unsigned nDisplay = 10;
    //printf("First %u elements",nDisplay);
    //for(int ii=0; ii<nDisplay; ii++) printf("  %u ",vals_dst[ii]);
    //printf("\n\n");
  ...
  // NOTE: after sorting, outputVals = inputVals and they are both in sorted order.
